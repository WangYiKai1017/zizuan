...
- **禁止编造**：只能记录用户明确讲出的事实；不要补充“技术骨干”“退休年份”“采访开始”等用户未明确表达的事件或评价
- **地点保守**：事件地点必须来自该事件上下文；不要把居住地当作职业事件地点。若事件地点不明确，填写 null
- **画像阶段**：如果输入只是首次画像信息，可以从用户明确列出的经历中建立简要事件条目，但不要创造“开始采访/准备自传”这类元事件
