...
- LLM 配置：src/config/llm_config.py 支持多提供商（Qwen、DeepSeek、OpenAI、Anthropic），优先读取 Qwen 专属变量，其次回退通用变量
- 默认配置：src/services/llm_service.py 通过 get_default_config 优先尝试 DeepSeek，失败回退 Qwen
...
- LLM 配置错误
  - 检查 .env 是否存在且包含 QWEN_URL/QWEN_APIKEY（或其他提供商对应变量）
