...
## 核心组件
- LLMConfig：基于 Pydantic 的配置模型，支持多提供商、环境变量覆盖、默认值与约束校验；提供 from_env/from_env_qwen/from_env_deepseek 等工厂方法，以及 get_default_config 的回退策略。
- LLMService：统一LLM调用入口，负责模型初始化（OpenAI/Anthropic/Qwen/DeepSeek兼容）、Prompt模板加载与渲染、结构化输出解析、重试与统计。
...
Env->>Cfg : 读取QWEN_URL/QWEN_APIKEY/LLM_*等
...
### LLMConfig 配置类设计与实现
- 多提供商支持
  - provider 支持 openai、anthropic、qwen、deepseek；其中 qwen 与 openai 兼容，deepseek 通过额外参数禁用思考模式。
  - 提供 from_env/from_env_qwen/from_env_deepseek 工厂方法，优先读取 QWEN_URL/QWEN_APIKEY 或 DEEPSEEK_URL/DEEPSEEK_APIKEY，否则抛出异常。
  - get_default_config 优先尝试 DeepSeek 配置，失败则回退到 Qwen。
- 参数与约束
...
- 环境变量覆盖
  - 通过 python-dotenv 加载 .env；优先使用 Qwen/DeepSeek 专属键，其次回退到通用键（LLM_*）。
- 运行时配置更新
...
### 配置加载机制与优先级
- 默认配置：get_default_config 优先尝试 DeepSeek，失败回退 Qwen。
- 环境变量覆盖：from_env/from_env_qwen/from_env_deepseek 优先读取专属键，若缺失则抛出异常，避免静默失败。
...
- 模型初始化：根据 provider 选择 ChatOpenAI 或 ChatAnthropic；Qwen 与 OpenAI 兼容，DeepSeek 通过额外参数禁用思考模式。
...
## 故障排查指南
- 环境变量未配置
  - 现象：from_env/from_env_qwen/from_env_deepseek 抛出异常。
  - 处理：确保 QWEN_URL/QWEN_APIKEY 或 DEEPSEEK_URL/DEEPSEEK_APIKEY 配置完整；或使用通用 LLM_* 键。
- LLM 调用失败
...
### 配置项说明与作用
- provider：模型提供商（openai/anthropic/qwen/deepseek）
- model_name：模型名称（如 qwen-plus、gpt-4o 等）
- api_key：提供商 API 密钥
...
- 环境变量：from_env/from_env_qwen/from_env_deepseek 优先专属键，其次通用键。
...
### 配置示例与常见场景
- 使用通义千问（推荐）
  - QWEN_URL、QWEN_APIKEY 配置完整，LLMService 将自动初始化 Qwen 模型。
- 使用 DeepSeek
