...
## 简介
本文件面向"服务配置管理"，聚焦于 LLMConfig 配置类的设计与实现，涵盖多提供商配置支持（OpenAI、Anthropic、通义千问、DeepSeek）、模型参数设置（provider、model_name、temperature、max_tokens、api_key、base_url 等）、连接池与重试机制、配置加载（默认配置、环境变量覆盖、运行时更新）、ProfileQuestions 用户画像问题库的设计原则与配置方法，以及配置最佳实践（安全、性能、故障排除）。文档同时给出配置示例与常见场景解决方案，帮助开发者快速、安全、稳定地部署与维护系统。

**重要更新**：根据最新的代码变更，`story_expectation` 字段已在系统中完全移除。这一变更影响了 ProfileQuestions 配置、ProfileCollectionAgent 的逻辑以及相关的字段映射配置。请参考更新后的配置说明和最佳实践。
...
**图表来源**
- [src/config/llm_config.py:10-127](file://src/config/llm_config.py#L10-L127)
- [src/config/profile_questions.py:3-88](file://src/config/profile_questions.py#L3-L88)
- [src/services/llm_service.py:32-599](file://src/services/llm_service.py#L32-L599)
- [src/agents/profile_collection_agent.py:14-548](file://src/agents/profile_collection_agent.py#L14-L548)
- [Prompts/ProfileCollection-Prompt.md:1-406](file://Prompts/ProfileCollection-Prompt.md#L1-L406)
...
**图表来源**
- [src/config/llm_config.py:42-127](file://src/config/llm_config.py#L42-L127)
- [src/services/llm_service.py:90-125](file://src/services/llm_service.py#L90-L125)
- [src/agents/profile_collection_agent.py:168-513](file://src/agents/profile_collection_agent.py#L168-L513)
...
**章节来源**
- [src/config/llm_config.py:113-127](file://src/config/llm_config.py#L113-L127)
- [src/services/llm_service.py:585-599](file://src/services/llm_service.py#L585-L599)
...
- 问题集合
  ...
  - DETAIL_QUESTIONS：family_status、children_count、living_arrangement、health_status、important_person、favorite_memory 等详细字段。
...
**重要更新**：`story_expectation` 字段已在系统中完全移除。ProfileQuestionBank 现已不再包含此字段，ProfileCollectionAgent 的必填字段列表也不再包含该字段。相关的字段映射和别名处理逻辑已被清理。
...
**图表来源**
- [src/services/llm_service.py:327-463](file://src/services/llm_service.py#L327-L463)
- [Prompts/ProfileCollection-Prompt.md:100-210](file://Prompts/ProfileCollection-Prompt.md#L100-L210)
...
## 结论
本配置体系以 LLMConfig 为核心，通过环境变量与工厂方法实现灵活的多提供商支持；LLMService 提供统一调用入口与模板管理，配合 ProfileCollectionAgent 与 ProfileQuestionBank 实现渐进式用户画像收集。随着 `story_expectation` 字段的移除，系统现在专注于核心的用户基本信息收集，简化了配置复杂度并提高了系统的稳定性。建议在生产环境中严格管理 API Key、合理设置温度与 Token 上限、启用重试与超时、完善日志与监控，并通过测试覆盖关键路径，确保稳定性与可维护性。
...
**重要更新**：`story_expectation` 字段已在系统中完全移除。ProfileQuestionBank 现在只包含基础信息和详细信息两类问题，不再包含故事期望相关的字段。ProfileCollectionAgent 的必填字段列表已更新为：name、age、occupation、family_status、living_arrangement。
...
**重要更新**：由于 `story_expectation` 字段的移除，画像收集流程现在只需要收集以下必填字段：name、age、occupation、family_status、living_arrangement。系统将不再询问用户的故事期望或记录目标。
...
### 字段映射与别名处理
- 字段规范化：系统支持多种字段别名的规范化处理，包括中文字段名到标准字段名的映射。
- 移除的字段：`story_expectation` 字段的别名映射已被移除，相关的字段映射逻辑已清理。
- 兼容性：系统仍保留对常见字段别名的支持，如姓名、年龄、职业等字段的多种表达方式。

**章节来源**
- [src/agents/profile_collection_agent.py:311-342](file://src/agents/profile_collection_agent.py#L311-L342)
- [tests/test_profile_collection_agent.py:82-106](file://tests/test_profile_collection_agent.py#L82-L106)
