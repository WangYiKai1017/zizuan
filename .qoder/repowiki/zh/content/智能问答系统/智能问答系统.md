...
## 更新摘要
**所做更改**
- 新增 GuidedInitialInterviewController 引导式初始面试控制器组件
- 增强 ProfileCollectionAgent 资料收集代理功能
- 添加引导式初始面试流程和改进的资料收集机制
- 更新架构图以反映新的组件关系
...
- [ProfileCollectionAgent:16-584](file://src/agents/profile_collection_agent.py#L16-L584)
...
- [ProfileCollectionAgent:161-204](file://src/agents/profile_collection_agent.py#L161-L204)
...
- 字段收集策略：
  ...
  - 可选字段：gender、birth_year、birth_place、children_count、health_status、important_person、favorite_memory、story_expectation
- 提取机制：
...
**图表来源**
- [ProfileCollectionAgent:124-204](file://src/agents/profile_collection_agent.py#L124-L204)
- [ProfileCollectionAgent:206-278](file://src/agents/profile_collection_agent.py#L206-L278)
...
- [SessionState:24-139](file://src/models/session_state.py#L24-L139)
- [ProfileCollectionAgent:124-159](file://src/agents/profile_collection_agent.py#L124-L159)
...
GIIC["GuidedInitialInterviewController"] --> LLM
...
- 资料收集提取失败：
  ...
  - 参考：[ProfileCollectionAgent:263-268](file://src/agents/profile_collection_agent.py#L263-L268)
- 会话时间到：
  - 现象：触发时间警告与结束引导
...
- [ProfileCollectionAgent:263-268](file://src/agents/profile_collection_agent.py#L263-L268)
- [ConversationOrchestrator:570-592](file://src/core/conversation_orchestrator.py#L570-L592)

## 结论
本系统通过"主控器 + 多服务 + 工具 + 模板"的分层设计，实现了高效、可扩展的智能问答与采访引导能力。新增的 GuidedInitialInterviewController 和增强的 ProfileCollectionAgent 进一步完善了系统的初始交互体验，提供了更加专业和人性化的面试流程。InterviewAgent 以 InterviewAgent 为核心，结合 QuestionGenerator 的多策略问题生成、EmotionDetector 的情绪感知与策略调整、KnowledgeBaseQuerier 的 ReAct 知识检索，以及完善的会话状态管理与缓存机制，形成了从"感知-决策-执行-反馈"的闭环。该架构既保证了实时性与稳定性，又具备良好的可维护性与扩展性。
...
- 代码实现路径示例（仅路径，不含具体代码内容）：
  ...
  - 资料收集代理：[ProfileCollectionAgent.handle_input:161-204](file://src/agents/profile_collection_agent.py#L161-L204)
