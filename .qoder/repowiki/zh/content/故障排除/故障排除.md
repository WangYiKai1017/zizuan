...
### LLM配置（LLMConfig）
- 环境变量优先策略：使用DeepSeek专属配置。
- 超时与重试：max_retries、retry_delay、timeout。
- 提供商支持：openai/anthropic/deepseek。
...
- 诊断步骤
  ...
  - 确认复制.env.example为.env并填写必要字段（如DEEPSEEK_URL/DEEPSEEK_APIKEY）
- 解决方案
...
### 配置错误
- 症状
  - LLM初始化失败或抛出“未配置DeepSeek专属API密钥或URL”
  - LLM调用超时或频繁重试
- 诊断步骤
  - 检查LLMConfig.from_env是否被调用
  - 验证DEEPSEEK_URL/DEEPSEEK_APIKEY是否设置
...
## 附录
- 快速检查清单
  - 环境变量：DEEPSEEK_URL/DEEPSEEK_APIKEY/LOG_LEVEL/LOG_FILE/KNOWLEDGE_BASE_PATH
