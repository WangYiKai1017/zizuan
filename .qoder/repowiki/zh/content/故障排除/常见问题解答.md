...
## 核心组件
- LLM 配置与调用
  - 通过 LLMConfig 从环境变量加载配置，支持 DeepSeek、OpenAI、Anthropic 等提供商；提供默认配置回退策略。
...
- 环境变量与 API 密钥
  - DeepSeek 专属配置优先：DEEPSEEK_URL、DEEPSEEK_APIKEY；若缺失则抛出配置错误。
  - 也可使用通用配置（如 LLM_PROVIDER、LLM_MODEL_NAME、LLM_API_KEY、LLM_BASE_URL 等），但默认优先 DeepSeek。
- 调用与重试
  - LLMService.invoke 支持系统提示、历史消息、指数退避重试与统计。
  - 结构化输出通过模板渲染与 JSON 解析，失败时返回错误结果。
- 常见问题与对策
  - 未配置 DEEPSEEK_URL/DEEPSEEK_APIKEY：将导致 LLMConfig.from_env 抛错。请复制 .env.example 并填入对应值。
...
- 环境变量未生效
  - 现象：DEEPSEEK_URL/DEEPSEEK_APIKEY 未被读取，导致配置加载失败。
...
### 错误信息解读与修复步骤
- 配置类错误
  - “未配置DeepSeek专属API密钥或URL”：请在 .env 中设置 DEEPSEEK_URL 与 DEEPSEEK_APIKEY。
...
- 安装与环境
  ...
  - .env 是否存在且包含 DEEPSEEK_URL/DEEPSEEK_APIKEY
- 运行与配置
  - LLMService 是否初始化成功
