...
## 更新摘要
**变更内容**
- 移除了 MarkdownFileManager 中对 story_expectation 字段的 profile 信息处理逻辑
- 更新了画像收集流程，不再依赖 story_expectation 字段进行话题选择
- 简化了 ProfileCollectionAgent 的字段规范化处理，移除了对 story_expectation 的特殊处理
- ConversationOrchestrator 中的话题选择逻辑不再依赖 story_expectation 字段
...
### ProfileCollectionAgent 画像收集流程
**更新** 移除了对 story_expectation 字段的处理逻辑

- 字段收集
  - 收集基础信息：姓名、年龄、职业、家庭状况、居住安排等14个字段。
  - 渐进式提问，自然对话，避免表格化提问。
  - 从用户回答中提取结构化信息，支持字段别名映射。
- 字段规范化
  - 支持多种字段别名，如"高寿"→"age"、"职业"→"occupation"等。
  - 移除了对 story_expectation 字段的特殊处理逻辑。
- 回退提取
  - 当 LLM 提取失败时，通过正则表达式回退提取明确信息。
  - 不覆盖已收集的字段，保守补充缺失信息。

**章节来源**
- [profile_collection_agent.py:16-548](file://src/agents/profile_collection_agent.py#L16-L548)

### ConversationOrchestrator 会话控制
**更新** 简化了话题选择逻辑，不再依赖 story_expectation 字段

- 话题选择
  - 根据用户画像选择第一个话题，优先考虑用户期望。
  - 移除了对 story_expectation 字段的依赖，改为更通用的选择逻辑。
  - 当用户年龄≥80岁时，优先选择童年记忆话题。
  - 当用户有职业信息时，选择相关的职业经历话题。
- 会话状态管理
  - 协调所有子Agent异步工作。
  - 管理对话状态流转，处理用户输入并生成回复。
  - 触发内容归纳和交接。
...
- ProfileCollectionAgent 依赖 LLMService 和 MemoryManager，负责画像收集。
- ConversationOrchestrator 依赖多个组件，负责整体会话控制。
...
PCA --> MM
CO["ConversationOrchestrator"] --> PCA
CO --> MM
```
...
- 画像收集优化
  - 移除 story_expectation 字段处理后，减少了不必要的字段映射开销。
  - 简化的字段规范化逻辑提高了画像收集的处理效率。
...
- 画像收集字段处理异常
  - 现象：字段映射错误或遗漏。
  - 排查：检查 ProfileCollectionAgent 的字段别名映射；确认回退提取逻辑正常工作。
  - 参考：[profile_collection_agent.py:311-342](file://src/agents/profile_collection_agent.py#L311-L342)
- 会话话题选择问题
  - 现象：话题选择不符合预期。
  - 排查：检查 ConversationOrchestrator 的话题选择逻辑；确认用户画像字段完整性。
  - 参考：[conversation_orchestrator.py:1301-1314](file://src/core/conversation_orchestrator.py#L1301-L1314)
...
- [memory_repository.py:401-447](file://src/storage/memory_repository.py#L401-L447)
- [profile_collection_agent.py:311-342](file://src/agents/profile_collection_agent.py#L311-L342)
- [conversation_orchestrator.py:1301-1314](file://src/core/conversation_orchestrator.py#L1301-L1314)
...
**更新** 本次更新移除了对 story_expectation 字段的处理逻辑，简化了画像收集和会话控制流程，提高了系统的简洁性和维护性。ProfileCollectionAgent 的字段规范化逻辑更加专注于核心字段的处理，ConversationOrchestrator 的话题选择逻辑也变得更加通用和可靠。
