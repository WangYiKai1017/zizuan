...
- [story_generation_agent.py](file://src/agents/story_generation_agent.py)
- [story_runner.py](file://src/service/agent_runners/story_runner.py)
...
### HTTP 任务 Runner 与 SSE
- SSEEmitter
  - 功能：统一输出 SSE 事件与 JSON data
  - 使用场景：采访、知识库整理、故事生成、传记大纲、传记写作
- StoryRunner
  - 功能：包装 StoryGenerationAgent，输出 task_started、scanning、generating、saved、completed、failed、done
- KBOrganizerRunner
  - 功能：执行知识库整理任务，并持久化最近一次 result
- OutlineRunner / WritingRunner
  - 功能：包装传记大纲生成与章节写作任务
- Observability
  - 功能：提供 route/request 级 Langfuse observation 上下文

章节来源
- [sse_response.py:1-76](file://src/service/sse_response.py#L1-L76)
- [story_runner.py:30-111](file://src/service/agent_runners/story_runner.py#L30-L111)
- [kb_organizer_runner.py:56-139](file://src/service/agent_runners/kb_organizer_runner.py#L56-L139)
- [outline_runner.py:1-76](file://src/service/agent_runners/outline_runner.py#L1-L76)
- [writing_runner.py:1-123](file://src/service/agent_runners/writing_runner.py#L1-L123)
- [observability.py:1-252](file://src/services/observability.py#L1-L252)

### 故事生成服务接口
- StoryGenerationAgent
  - load_unconsumed_events(): 读取未被故事消费的 event
  - select_events(events): 选择最早 15 个 event
  - generate_story(events): 调用 LLM 生成第一人称故事
  - save_story_and_mark_consumed(story, events): 保存故事并更新 `.story_state.json`

章节来源
- [story_generation_agent.py:78-306](file://src/agents/story_generation_agent.py#L78-L306)
- [story_runner.py:30-111](file://src/service/agent_runners/story_runner.py#L30-L111)
...
- 提供多种 from_env_* 方法：
  - from_env(): 通用配置（可选）
  - from_env_qwen(): 优先使用 Qwen 专属配置（推荐）
  - from_env_deepseek(): 使用 Deepseek 专属配置
- 默认配置 get_default_config(): 优先 DeepSeek，失败回退 Qwen
- 环境变量键：
  - 通用：LLM_PROVIDER、LLM_MODEL_NAME、LLM_API_KEY、LLM_BASE_URL、LLM_TEMPERATURE、LLM_MAX_TOKENS
  - Qwen专属：QWEN_URL、QWEN_APIKEY
