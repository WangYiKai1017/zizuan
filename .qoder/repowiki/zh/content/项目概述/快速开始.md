...
## 核心组件
- LLM 配置与服务
  - LLMConfig：从环境变量加载 LLM 提供商、模型名、API Key、基础 URL、温度、最大 Token 等参数；支持 Qwen/DeepSeek/OpenAI/Antropic 等提供商。
...
### 环境变量配置
- LLM 提供商配置
  - 优先使用 Qwen 专属配置（QWEN_URL、QWEN_APIKEY），若不可用则回退到通用配置
...
- 初始化 LLM 服务
  - 通过 LLMConfig.from_env 或 from_env_deepseek 加载配置
  - LLMService 构造时自动初始化模型（OpenAI/Qwen/DeepSeek/Antropic）
- 创建 InterviewAgent
...
## 故障排查指南
- 环境变量未配置
  - 确认 .env 文件存在并包含 QWEN_URL/QWEN_APIKEY 或 DEEPSEEK_URL/DEEPSEEK_APIKEY
  - 若缺失，LLMConfig.from_env/from_env_deepseek 将抛出异常
- LLM 调用失败
...
- 配置环境变量
  - cp .env.example .env
  - 编辑 .env，填入 QWEN_URL/QWEN_APIKEY 等
- 运行测试
  - pytest
  - pytest -v
