...
## 核心组件
- LLMConfig：基于Pydantic的配置模型，负责从环境变量加载并进行类型与范围校验；支持Qwen与DeepSeek专属配置优先策略，并提供默认配置回退逻辑。
...
- 环境变量来源与优先级
  - 全局：项目启动时加载.env文件
  - 专属优先：优先检测Qwen专属配置（QWEN_URL、QWEN_APIKEY），若存在则使用Qwen配置并填充其他字段
  - DeepSeek专属：提供from_env_deepseek方法，优先检测DEEPSEEK_URL、DEEPSEEK_APIKEY
  - 默认回退：当DeepSeek不可用时，回退到Qwen专属配置
- 错误处理
...
- 支持OpenAI/Qwen（兼容OpenAI API）、DeepSeek、Anthropic等
- 统一封装调用、错误处理与日志记录
...
## 故障排查指南
- 缺少专属API密钥或URL
  - 现象：from_env/from_env_qwen/from_env_deepseek抛出异常
  - 处理：补充QWEN_URL/QWEN_APIKEY或DEEPSEEK_URL/DEEPSEEK_APIKEY
- provider不受支持
  - 现象：LLMService初始化时报错
  - 处理：确认provider在支持列表内（openai/qwen/deepseek/anthropic）
- .env未生效
  - 现象：环境变量读取为空
...
### 不同环境的配置模板与差异
- 开发环境
  - 使用本地或测试服务端点（QWEN_URL/DEEPSEEK_URL）
  - 降低超时与重试次数，便于快速反馈
- 测试环境
  - 使用稳定的服务端点与较小的并发
  - 启用详细日志以便复现问题
